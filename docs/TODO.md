...
- `SharedLibrary::setLibraryPath()` on **macOS**. On Linux it was measured that setting `LD_LIBRARY_PATH` at run time does not change where this process looks for a dependency, since glibc reads the variable once while the process is starting: a library whose dependency sits only in the new directory still fails to load after the call, and the same binary loads it when the variable was set before it started. The header says the same of `DYLD_LIBRARY_PATH` because dyld is not glibc and there was no mac to ask. Run that probe there and either confirm the wording or replace it. The same visit should check `PreventUnloadHint`, since `RTLD_NODELETE` is defined on Darwin and the case for it has only ever run on Linux and Windows.
