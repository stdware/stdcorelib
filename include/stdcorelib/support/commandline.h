...
namespace stdc::cli {
    ...
    public:
        ...
        /// Whether every command in this tree can tell its own names apart.
        ///
        /// What is in scope at a command is its own options plus the recursive ones above it,
        /// and they are looked up by spelling, so two of them answering to one spelling has no
        /// right answer. The pieces cannot see this as they are added: a command knows nothing
        /// about the ancestors it will end up under, so the whole tree is asked once it is one.
        ///
        /// \a ignoreOptionCase and \a ignoreCommandCase are the options the line will be read
        /// with, since under those two spellings differing only in case are one spelling. Parser
        /// asserts this without them where it is handed a tree and with them where it parses.
        static inline bool namesAreUnambiguous(const Command &command,
                                               bool ignoreOptionCase = false,
                                               bool ignoreCommandCase = false) {
            std::vector<const Option *> inherited;
            return unambiguousUnder(command, inherited, ignoreOptionCase, ignoreCommandCase);
        }
    ...
    private:
        /// \a command's own names against \a inherited, then the same for what is under it,
        /// carrying whatever it declares recursive down with it.
        static inline bool unambiguousUnder(const Command &command,
                                            std::vector<const Option *> inherited,
            ...
            const auto &take = [&spellings, ignoreOptionCase](const Option &option) {
                for (const auto &spelling : option.tokens()) {
                    for (const auto &taken : spellings) {
                        if (detail::same_name(taken, spelling, ignoreOptionCase)) {
                            return false;
                        }
                    }
            ...
            for (const auto &sub : command.commands()) {
                for (const auto &taken : names) {
                    if (detail::same_name(taken, sub.name(), ignoreCommandCase)) {
                        return false;
                    }
                }
            ...
            for (const auto &sub : command.commands()) {
                if (!unambiguousUnder(sub, inherited, ignoreOptionCase, ignoreCommandCase)) {
                    return false;
                }
            }
            return true;
